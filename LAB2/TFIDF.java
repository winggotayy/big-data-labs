    ...
    public static class TReducer extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text word, Iterable<Text> value, Context context) throws IOException, InterruptedException {
            ...
            for (Text i : value) {
                ...
                if (tfMap.containsKey(bookName)) {
                    int currentCount = tfMap.get(bookName);
                    tfMap.put(bookName, currentCount + Integer.parseInt(buf[1]));
                } else {
                    tfMap.put(bookName, Integer.parseInt(buf[1]));
                }
            }
            int docnum = tfMap.size();
            double idf = calculateIDF(total, docnum);
        ...
        private double calculateIDF(int totalDocs, int docsWithTerm) {
            return Math.log10((double) totalDocs / (docsWithTerm + 1));
        }
    }
